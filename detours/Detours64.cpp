...
namespace Detours
{
	namespace X64
	{
		...
		bool DetourRemove(uintptr_t Trampoline)
		{
			...
			auto HeaderUnaligned = Trampoline - sizeof(JumpTrampolineHeader);
			
			HeaderUnaligned -= (HeaderUnaligned % 64);

			auto header = reinterpret_cast<JumpTrampolineHeader *>(HeaderUnaligned);
